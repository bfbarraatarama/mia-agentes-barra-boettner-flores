...
El agente base es `MyAgent`, implementado en `student_framework/agent.py`. El baseline inicial de M3 conserva el loop ReAct desarrollado en M1/M2, utilizando la misma lógica de interacción entre el LLM y las tools y la misma semántica de finalización. Sobre esta base se incorporan posteriormente mecanismos opcionales —como reparación de tool calls y gestión de contexto— y una variante PlannerAgent, lo que permite evaluar cada extensión respecto de una referencia común.
...
- **por éxito**: consumo acumulado de todos los trials dividido por la cantidad de trials exitosos, para cuantificar el costo efectivo de producir un éxito. Estas métricas incluyen por lo tanto el consumo de los trials fallidos y no representan el consumo promedio condicionado únicamente a los trials exitosos.
...
### 6.9 Selección de candidatos para la validación final

Como cierre de la etapa exploratoria se construyó un ranking común de las condiciones evaluadas en la línea experimental final. Cada sistema se contabiliza una única vez a partir de la evidencia persistida correspondiente, y la figura se regenera mediante [`scripts/informe_m3_figures.py`](../scripts/informe_m3_figures.py).

![Ranking de los cinco mejores sistemas observados](recursos/m3_system_ranking.svg)

Los cinco mejores resultados observados son `planner + recovery` (78,8%), `baseline` (73,8%), `planner` (72,5%), `baseline_incremental + recovery` (70,0%) y `planner_incremental + recovery` (68,8%).

Para la selección final adoptamos `multi_attempt_recovery` como requisito del protocolo. La recuperación había sido promovida como nueva referencia después de observar que 33 trials alcanzaron el objetivo luego de abrir efectivamente una recuperación, y las intervenciones posteriores se diseñaron sobre esa misma base. Por este motivo, `baseline` y `planner` sin recovery permanecen en el ranking como resultados históricos de la línea experimental, pero no se consideran candidatos para la configuración final.

Al aplicar este criterio, los tres candidatos con mayor success observado son `planner + recovery`, `baseline_incremental + recovery` y `planner_incremental + recovery`, con 78,8%, 70,0% y 68,8%, respectivamente. Las diferencias respecto del primero son 8,8 y 10,0 puntos porcentuales, valores que permanecen dentro o en el borde del rango que tratamos con cautela a partir de la variabilidad observada entre runs.

Por este motivo, la selección definitiva se reserva para una **run de validación final independiente**. La nueva corrida mantiene `nova-lite`, `multi_attempt_recovery` y los ocho escenarios, incluye únicamente estos tres candidatos y aumenta la cantidad de repeticiones a **20 trials por escenario**, equivalentes a 160 trials por sistema y 480 trials en total. Esta corrida se utilizará además como fuente de la evaluación cualitativa automática final, de modo que la comparación cuantitativa y la cualitativa queden trazadas hacia una misma evidencia primaria.

---

## 7. Validación y selección final

La validación final se ejecutó como [`m3-final-run-008`](../eval/results/runs/m3-final-run-008.manifest.json) sobre los tres candidatos seleccionados en la etapa anterior:

- `planner + recovery`;
- `baseline_incremental + recovery`;
- `planner_incremental + recovery`.

Los tres utilizan `nova-lite`, `multi_attempt_recovery`, los mismos ocho escenarios y 20 trials por escenario. El run contiene por lo tanto **480 trials independientes, 160 por sistema**, y su manifest registra el commit `ec09522` con `dirty: false`.

La evaluación cuantitativa correspondiente es [`m3-final-eval-008`](../eval/results/evaluations/m3-final-eval-008/results.json). A diferencia de las comparaciones exploratorias anteriores, esta corrida no estudia una intervención nueva: su objetivo es volver a medir bajo una misma ejecución y con mayor cantidad de repeticiones los tres sistemas que habían quedado como candidatos.

### 7.1 Desempeño cuantitativo

![Tasa de éxito de los tres candidatos en la validación final](recursos/m3_final_selection_success.svg)

La nueva evidencia modifica el orden observado durante la etapa exploratoria. Globalmente se obtienen:

- `planner + recovery`: **106/160 = 66,3%**;
- `baseline_incremental + recovery`: **113/160 = 70,6%**;
- `planner_incremental + recovery`: **114/160 = 71,3%**.

En particular, `planner + recovery`, que había alcanzado 78,8% y encabezaba el ranking utilizado para seleccionar los candidatos, queda tercero en la nueva corrida. Este cambio refuerza la cautela adoptada previamente frente a diferencias moderadas obtenidas sobre runs independientes: el mayor valor observado en una corrida exploratoria no garantiza conservar el mismo orden al repetir la evaluación.

Tampoco aparece un ganador claro entre las dos variantes incrementales. `planner_incremental` supera a `baseline_incremental` por **un solo trial sobre 160**, equivalente a 0,6 puntos porcentuales. Esa diferencia es demasiado pequeña para fundamentar por sí sola una preferencia entre ambos sistemas.

El desglose por escenario muestra además que ninguna configuración domina uniformemente. `planner_incremental` alcanza 95% en `office-sequence`, frente a 70% de los otros dos candidatos, y empata el 95% de `baseline_incremental` en `apartment-keys`. En cambio, `planner` obtiene 100% en `extreme-archive`, y `baseline_incremental` conserva mejores resultados en `vault-combination`. La validación final vuelve así a mostrar que el agregado global resume comportamientos heterogéneos entre tareas.

Por lo tanto, el success rate permite descartar una lectura basada en el ranking exploratorio, pero **no alcanza para separar con claridad a los dos candidatos incrementales**. La decisión final requiere incorporar las demás dimensiones de evaluación.

### 7.2 Costo y eficiencia

El [análisis de eficiencia de `m3-final-eval-008`](../eval/results/evaluations/m3-final-eval-008/efficiency_analysis.md) permite comparar el consumo a partir de la misma evidencia utilizada para el success rate.

![Consumo de tokens por trial en la validación final](recursos/m3_final_selection_cost.svg)

El costo global promedio por trial es:

- `planner + recovery`: **186,6k tokens/trial**;
- `baseline_incremental + recovery`: **205,9k tokens/trial**;
- `planner_incremental + recovery`: **175,9k tokens/trial**.

A diferencia del success rate, acá aparece una separación más clara. `planner_incremental` utiliza aproximadamente **14,6% menos tokens por trial que `baseline_incremental`** y **5,7% menos que `planner`**.

La misma tendencia se mantiene cuando el consumo se relaciona con los éxitos obtenidos. El consumo efectivo por éxito es aproximadamente **246,8k tokens** para `planner_incremental`, **281,6k** para `planner` y **291,5k** para `baseline_incremental`. Esta métrica divide el consumo acumulado de todos los trials, incluidos los fallidos, por la cantidad de éxitos observados; no representa el consumo promedio condicionado únicamente a los trials exitosos.

El desglose por escenario vuelve a mostrar heterogeneidad. Por ejemplo, `planner_incremental` presenta un consumo especialmente favorable en `office-sequence` y `backtracking-vault`, mientras que `baseline_incremental` resulta menos costoso en `study-with-key`, `apartment-keys`, `extreme-archive` y `vault-combination`. El agregado no implica entonces que una variante sea siempre la más barata, sino que **sobre los 160 trials completos `planner_incremental` ofrece el menor consumo total promedio de los tres candidatos**.

Este resultado adquiere relevancia precisamente porque los dos sistemas con mayor success rate quedan prácticamente empatados. Entre `baseline_incremental` y `planner_incremental`, la diferencia de éxito es de sólo un trial, mientras que la diferencia de consumo agregado es considerablemente mayor.

### 7.3 Evaluación cualitativa

La evaluación cualitativa final utiliza la misma evidencia primaria de `m3-final-run-008`. El dataset [`m3-final-selection-qualitative-v1`](../eval/results/llm_judge/m3-final-selection-qualitative-v1/manifest.json) toma aleatoriamente **tres trials de cada escenario para cada sistema**, sin reemplazo y con `seed=20260826`. De esta manera cada sistema aporta 24 trayectorias y la evaluación contiene **72 casos en total**.

El tamaño de la muestra se fija por sistema y escenario, no como una proporción del número total de trials. Esto mantiene representados los ocho escenarios y permite que el costo de una evaluación cualitativa futura permanezca controlado aunque aumente la cantidad de ejecuciones cuantitativas disponibles. No se fuerza un balance entre éxitos y fallos: dentro de cada combinación sistema-escenario los casos se seleccionan al azar entre los trials disponibles.

Sobre estos casos se aplica, sin una nueva etapa de calibración, la configuración del judge ya validada en [Validación del LLM-as-judge](#56-validación-del-llm-as-judge). El [manifest de `m3-final-selection-judge-001`](../eval/results/llm_judge/m3-final-selection-qualitative-v1/judge_evaluations/m3-final-selection-judge-001/manifest.json) conserva `planning-quality-v5`, el prompt congelado y Claude Opus 4.5 con temperatura 0. Los resultados agregados se preservan en [`system_summary.json`](../eval/results/llm_judge/m3-final-selection-qualitative-v1/judge_evaluations/m3-final-selection-judge-001/system_summary.json).

![Calidad de la planificación durante la trayectoria en la validación final](recursos/m3_final_selection_qualitative.svg)

La figura conserva separados los cuatro criterios de la rúbrica y agrega `Q1 Promedio` como resumen descriptivo. Este valor es el promedio simple de las tasas de PASS de Q1.1 a Q1.4; no constituye un quinto criterio de la rúbrica ni se utiliza como score decisorio.

`planner` obtiene los mayores valores en Q1.1, con 19/24 PASS (79,2%), y comparte con `baseline_incremental` el máximo de Q1.2, con 20/24 (83,3%). También alcanza 16/24 en Q1.3 (66,7%). Su resultado más bajo aparece en Q1.4, con 9/18 casos aplicables (50,0%).

`baseline_incremental` presenta un perfil similar en Q1.1 y Q1.2, con 18/24 (75,0%) y 20/24 (83,3%), pero cae a 14/24 (58,3%) en consistencia de la ejecución con la estrategia y a 10/21 (47,6%) en monitoreo y replanificación.

`planner_incremental` no domina todos los criterios: obtiene 17/24 (70,8%) en Q1.1 y 19/24 (79,2%) en Q1.2, por debajo de los máximos observados. Sin embargo, empata el mayor resultado de Q1.3 con 16/24 (66,7%) y obtiene el valor más alto de Q1.4, con **13/21 casos aplicables (61,9%)**.

Los denominadores diferentes de Q1.4 son deliberados. Como se definió en la rúbrica, este criterio sólo se evalúa cuando la trayectoria contiene una oportunidad observable de adaptación. Por este motivo, las proporciones de Q1.4 se calculan únicamente sobre los casos aplicables y no sobre los 24 trials de cada sistema.

La evaluación cualitativa tampoco produce entonces una dominancia completa. `planner` resulta más fuerte en consistencia factual, mientras que `planner_incremental` presenta la mejor señal en monitoreo y replanificación y mantiene el mismo resultado que `planner` en la consistencia entre ejecución y estrategia. Como resumen visual, `Q1 Promedio` resulta 69,8% para `planner`, 66,1% para `baseline_incremental` y 69,6% para `planner_incremental`. La cercanía entre los dos primeros valores refuerza que este promedio se utilice únicamente como ayuda de lectura y que la interpretación cualitativa conserve el perfil separado de los cuatro criterios.

### 7.4 Sistema seleccionado

La validación final no identifica un sistema que sea superior en todas las dimensiones, por lo que la selección no se basa en una única métrica.

Por success rate, `planner_incremental + recovery` y `baseline_incremental + recovery` quedan prácticamente empatados: **71,3% frente a 70,6%**, una diferencia de apenas un trial sobre 160. `planner + recovery` alcanza 66,3%, y la nueva corrida no reproduce la ventaja que había mostrado durante la etapa exploratoria.

Por consumo, en cambio, `planner_incremental + recovery` presenta el mejor resultado global: **175,9k tokens/trial**, frente a 186,6k de `planner` y 205,9k de `baseline_incremental`. También obtiene el menor consumo efectivo por éxito, con aproximadamente **246,8k tokens por éxito**.

Cualitativamente no existe una dominancia completa. `planner_incremental` queda por debajo de los mejores resultados en Q1.1 y Q1.2, empata el máximo de Q1.3 y obtiene el mayor valor de Q1.4. Por lo tanto, la evidencia cualitativa no justifica afirmar que su planificación sea globalmente mejor, pero tampoco introduce una desventaja que contrarreste su desempeño cuantitativo y su menor consumo.

A partir de este balance se selecciona como configuración final:
...
La elección **no se fundamenta en los 0,6 puntos porcentuales de ventaja de success rate**, que consideramos insuficientes para distinguir los dos primeros sistemas. Se fundamenta en que `planner_incremental + recovery` combina:

- el mayor success rate observado en la validación final, prácticamente empatado con `baseline_incremental`;
- el menor consumo promedio por trial y por éxito entre los tres candidatos;
- un desempeño cualitativo competitivo, con el mejor resultado observado en monitoreo y replanificación y el mejor resultado compartido en consistencia entre ejecución y estrategia.

En términos de diseño, la configuración seleccionada combina tres decisiones que habían sido estudiadas durante M3: una planificación inicial explícita, instrucciones de ejecución incremental y recuperación externa frente a terminaciones controladas. La evidencia no permite atribuir el resultado final a una de ellas de forma aislada, porque la selección evalúa la política completa. Los experimentos anteriores son los que permiten interpretar por separado los efectos y tradeoffs observados durante su desarrollo.

La selección debe entenderse por lo tanto como una **decisión de ingeniería basada en múltiples dimensiones de la evidencia disponible**, no como una demostración estadística de superioridad universal. Sobre esta población y este protocolo, `planner_incremental + recovery` ofrece el balance más favorable entre cumplimiento de tarea, consumo y calidad observable de la trayectoria.

---

## 8. Limitaciones y trabajo futuro

La principal limitación experimental observada es la **variabilidad entre ejecuciones independientes**. Con diez trials por escenario se obtuvieron diferencias apreciables incluso entre configuraciones que no llegaron a diferir conductualmente durante una corrida. La validación final aumentó el tamaño a veinte trials por escenario y aun así los dos sistemas con mayor success rate quedaron separados por un único éxito sobre 160, mientras que `planner + recovery` pasó de 78,8% en la etapa exploratoria a 66,3% en la validación independiente. Como trabajo futuro convendría aumentar nuevamente la cantidad de repeticiones y complementar los valores puntuales con medidas explícitas de incertidumbre. Esto permitiría distinguir con mayor fundamento diferencias reales entre sistemas de fluctuaciones atribuibles al muestreo.

Una segunda limitación es que los experimentos exploraron **familias de estrategias, pero no sus hiperparámetros de forma exhaustiva**. En general se eligió una configuración representativa para cada intervención y se evaluó su comportamiento end-to-end. La ejecución incremental se estudió mediante una única variante del system prompt; el planner utilizó una única formulación de planificación y guidance; y las estrategias de summarization se evaluaron con schemas y prompts concretos. Del mismo modo, el trigger por tokens se fijó en 8.000 tokens y la revisión estratégica periódica en 20 mensajes, sin realizar una búsqueda sobre distintos valores. Algunas intervenciones, como `strategic_v1` combinado con periodicidad y trigger por tokens, son además compuestas y no permiten aislar el aporte de cada componente. Una extensión natural sería realizar ablaciones más finas y búsquedas sistemáticas de hiperparámetros dentro de las familias que resulten más prometedoras.

El **costo computacional de las trayectorias largas** constituye también una limitación práctica del pipeline experimental. Las sucesivas llamadas al proveedor vuelven a incluir gran parte de un contexto conversacional que permanece sin cambios entre una inferencia y la siguiente. La implementación actual no aprovecha mecanismos de caching de prefijos de contexto. Incorporarlos cuando el modelo y el proveedor utilizados lo permitan podría reducir tanto el costo de entrada como el tiempo necesario para ejecutar experimentos extensos, particularmente en escenarios largos y en configuraciones donde se acumulan muchos attempts o rondas ReAct. Para evaluar esa optimización sería conveniente persistir también métricas que permitan distinguir el consumo lógico del contexto del costo y la latencia efectivos introducidos por el backend.

La evaluación cualitativa presenta una limitación adicional. El LLM-as-judge fue calibrado y validado contra anotaciones humanas, alcanzando 90,0% de agreement y κ = 0,769 en holdout, pero la referencia humana utilizada para esta meta-evaluación continúa siendo pequeña. En particular, las estimaciones por criterio se apoyan en pocos casos y Q1.1 conserva una concordancia menor que los demás criterios. Una ampliación del conjunto anotado manualmente permitiría estimar con mayor precisión la confiabilidad del judge y estudiar sus fronteras de interpretación sobre una diversidad mayor de trayectorias.

Finalmente, la selección del sistema se realizó sobre **ocho escenarios de una misma batería y utilizando `nova-lite` como modelo final**. Los experimentos muestran que el modelo base modifica fuertemente tanto el success rate como la naturaleza de los fallos, por lo que no puede asumirse que el orden entre estrategias se conserve al cambiar de LLM o de distribución de tareas. Una validación posterior sobre escenarios nuevos, no utilizados durante el desarrollo de las intervenciones, y eventualmente sobre otros modelos con capacidad suficiente para resolver el dominio permitiría evaluar mejor la generalización de las conclusiones obtenidas.

---

## 9. Conclusión

M3 extendió el agente desarrollado en M1/M2 con una infraestructura reproducible para generar, persistir y analizar evidencia experimental sobre agentes autónomos en escape rooms. La evaluación combina ground truth sobre el estado real del entorno, métricas de éxito y consumo, análisis de errores y contexto, experimentos controlados sobre distintas estrategias y una evaluación cualitativa mediante LLM-as-judge previamente contrastada con anotaciones humanas.

Los experimentos muestran que no todas las extensiones del framework producen una mejora end-to-end. La selección del modelo resultó determinante para establecer una base capaz de resolver los escenarios; la reparación de tool calls corrigió errores locales cuando efectivamente aparecieron, pero no compensó otras limitaciones; el planner explícito mostró beneficios localizados sin una ventaja agregada clara; y las estrategias de summarization permitieron controlar distintos aspectos de la presión de contexto sin desplazar a las mejores configuraciones por success rate. La recuperación entre attempts, en cambio, mostró capacidad observable de rescatar trayectorias que habían alcanzado terminaciones controladas y se mantuvo como parte del protocolo final.

La validación independiente de los tres candidatos finales confirmó además la importancia de no seleccionar un sistema a partir de una única corrida. `planner_incremental + recovery` y `baseline_incremental + recovery` quedaron prácticamente empatados en success rate, mientras que el primero presentó un menor consumo global y un perfil cualitativo competitivo. Sobre esa evidencia se seleccionó `planner_incremental` con `nova-lite` y `multi_attempt_recovery` como la configuración que ofrece el mejor balance observado entre cumplimiento de tarea, eficiencia y calidad de planificación.

Más allá de la configuración seleccionada, el resultado principal de M3 es disponer de un proceso experimental trazable que permite formular una intervención, ejecutarla sobre evidencia reproducible, medir sus efectos desde distintas dimensiones y conservar tanto resultados favorables como negativos. Las limitaciones observadas señalan el paso siguiente: aumentar la robustez estadística de las comparaciones, explorar de forma más sistemática el espacio de configuraciones y reducir el costo operativo de ejecutar evaluaciones cada vez más extensas.