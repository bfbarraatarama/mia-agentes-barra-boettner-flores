...
Separadamente del reporting automático de una evaluation, `informes/scripts/plot_experiment_comparison.py` construye las figuras comparativas entre **condiciones experimentales ya persistidas** utilizadas en este informe. La función `plot_experiment_comparison` queda disponible como helper reutilizable para comparar cualquier par de condiciones y sistemas homólogos, mientras que la ejecución directa del script regenera de forma declarativa todas las comparaciones incorporadas al informe. En ambos casos se compara por sistema y escenario tanto el `success_rate` como el consumo promedio de tokens por trial, contrastando cada intervención con la etapa inmediatamente anterior.
...
### 6.8 Criterio para el trigger de summarization por tokens
...
La condición candidata mantiene la recuperación entre attempts de `m3-final-run-002` y elimina el prompt de ejecución incremental, que no fue promovido en §6.7. Como el trigger sólo puede afectar a los sistemas que utilizan summarization, `m3-final-run-006` evalúa únicamente:

- `summary_token_trigger`;
- `planner_summary_token_trigger`.

Sus controles homólogos son respectivamente `summary` y `planner_summary` de `m3-final-run-002`. Con los ocho escenarios y diez trials por escenario, el nuevo run contiene **160 trials**. Las variantes sin summarization no se repiten porque el trigger no modifica su comportamiento.
...
El cambio sí modifica fuertemente la dinámica de contexto. En `summary`, los trials con terminaciones por presupuesto pasan de 35/80 a 1/80 y los eventos de compactación de 693 a 81. En `planner_summary`, esos valores pasan de 30/80 a 1/80 y de 651 a 83. El trigger reactivo evita así compactar reiteradamente por una ventana artificialmente pequeña y prácticamente elimina las terminaciones asociadas a presión de contexto.

Esta reducción de compactaciones no produce, sin embargo, una reducción del costo total. `summary` pasa de aproximadamente 148,0k a 172,9k tokens por trial (+16,9%) y `planner_summary` de 148,5k a 187,2k (+26,0%). Agregando ambos sistemas, el costo aumenta de 148,2k a 180,0k tokens/trial (**+21,5%**). El costo de las llamadas de compactación disminuye, pero la ventana más amplia hace que las llamadas del agente procesen historiales mayores y ese incremento domina el balance final. Las dos variantes quedan además por encima de los 139,3k tokens/trial de `planner + recovery`.

**Qué concluimos.** El trigger de 8.000 `input_tokens` resulta efectivo como mecanismo de **gestión de contexto**: reduce drásticamente la cantidad de compactaciones y casi elimina las terminaciones por presupuesto. Esa mejora mecánica no se traduce en una mejora suficiente del objetivo final. El success agregado aumenta sólo 2,5 pp entre los sistemas afectados y el costo crece 21,5%; además, el mejor candidato permanece 15,0 pp por debajo de `planner + recovery` y es más costoso. Por lo tanto, esta política no se promueve como sistema final, aunque el experimento confirma que separar el trigger de compactación del límite físico del historial es una forma efectiva de controlar la presión de contexto.
### 6.9 Summary estratégico y revisión periódica
...
Como en §6.8, las barras representan los controles homólogos y la línea sólida conserva como benchmark a `planner + recovery` (**78,8%**). La intervención no produce una mejora global: `summary_strategic` aumenta 2,5 pp, pero `planner_summary_strategic` cae 5,0 pp. Existen efectos locales favorables —por ejemplo, `office-sequence` pasa de 60% a 80% y 90%, y `extreme-archive` de 10% a 40% y 20%—, pero no se generalizan al conjunto de escenarios.
...
A diferencia del trigger exclusivamente reactivo, esta estrategia no introduce un aumento global importante de costo. `summary_strategic` pasa de 148,0k a 138,2k tokens/trial (−6,6%), mientras que `planner_summary_strategic` pasa de 148,5k a 152,9k (+2,9%). Agregando ambos sistemas, el costo disminuye ligeramente de 148,2k a 145,5k tokens/trial (**−1,8%**). `summary_strategic` alcanza incluso un costo prácticamente igual al benchmark `planner + recovery` —138,2k frente a 139,3k tokens/trial—, pero con 58,8% de success frente a 78,8%.

La hipótesis de revisión estratégica tampoco queda respaldada por las métricas de repetición. La proporción de acciones repetidas dentro de un trial cambia sólo de 52,1% a 49,7% para `summary` y de 49,4% a 50,8% para `planner_summary`. Más aún, la proporción de acciones re-derivadas entre attempts aumenta de 53,4% a 63,5% y de 48,4% a 64,2%, respectivamente. Por lo tanto, las revisiones periódicas no muestran una reducción sistemática de los patrones repetitivos que motivaron la intervención.

**Qué concluimos.** La política estratégica y periódica es efectiva para controlar la presión de contexto y mantiene un costo agregado similar al de los controles, pero no mejora el success rate ni reduce de manera consistente la repetición o la re-derivación entre attempts. Tampoco alcanza al benchmark `planner + recovery`: su mejor variante queda 20,0 pp por debajo en success sin ofrecer una ventaja material de costo. En consecuencia, tampoco se promueve como sistema final. El resultado sugiere que, en estos escenarios, reducir la presión de contexto o introducir oportunidades explícitas de replanning no basta por sí solo para mejorar el comportamiento end-to-end del agente.
