...
### 5.6 Estado de la evaluación cualitativa

La dimensión cualitativa descrita en §4.6 está implementada de punta a punta —rúbrica `planning-quality-v4`, presentación ciega, anotación humana, juez LLM (`nova-pro`) y cálculo de acuerdo— pero su ejecución hasta ahora es una **calibración preliminar**, no un resultado sobre los sistemas del run final.

Lo corrido es el dataset `qualitative-llm-judge-smoke-v1`: 2 casos de un único escenario (`office-sequence`) y un único sistema, con 1 caso en dev y 1 en holdout. El acuerdo humano ↔ juez es de **0,50 con kappa 0,00** en ambos splits (n = 4 criterios por split), es decir acuerdo a nivel de azar. El patrón de desacuerdo es informativo y consistente: **Q1.3 (consistencia de la ejecución con la estrategia) desacuerda en los dos splits**, siempre en la misma dirección —el humano marca FAIL y el juez PASS— y con la misma fórmula en ambos casos: los errores "son corregidos en iteraciones posteriores y no afectan materialmente la estrategia general" (dev, citando las 40 acciones de la trayectoria como evidencia), "han sido corregidos y no han afectado materialmente la ejecución de la estrategia" (holdout). Es la regla de materialidad de la rúbrica aplicada como excusa general en lugar de como excepción acotada, y es el punto concreto a endurecer en la próxima versión de la rúbrica.

Por lo tanto, la calidad de planificación **no se reporta como resultado** en este informe. Lo que sí queda establecido es la infraestructura y un diagnóstico accionable sobre qué criterio hay que endurecer antes de escalar el dataset a los cuatro sistemas finales.

---

## 6. Experimentos

Se corrieron cuatro experimentos sobre qué partes del framework importan para este problema. Cada uno modifica un único aspecto y reutiliza la misma infraestructura de evaluación; los que no usan el run final se apoyan en evidencia archivada en `eval/results/historic/`, agrupada por condición experimental lógica según el mecanismo descrito en §2.5.
...
Lo que sí es un dato sólido es que los attempts adicionales aportan poco: el 50,5% de las acciones ejecutadas a partir del segundo attempt ya se habían ejecutado con idéntico resultado en un attempt anterior del mismo trial (§5.5). El agente conserva el historial y el estado del mundo, pero vuelve a derivar la mitad de lo que ya sabía.
